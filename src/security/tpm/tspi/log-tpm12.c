			      ...
			      int digests_len)
{
	...
	if (digests_len != 1) {
		printk(BIOS_WARNING, "TCPA: invalid number of digests: %d\n", digests_len);
		return;
	}
	if (digests->hash_type != VB2_HASH_SHA1) {
		printk(BIOS_WARNING, "TCPA: digest is of unsupported type: %s\n",
		       vb2_get_hash_algorithm_name(digests->hash_type));
		return;
	}
	...
	memcpy(tce->digest, digests->hash, TCPA_DIGEST_MAX_LENGTH);
